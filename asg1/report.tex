					...
					\item 
						...
						\begin{align*}
							...
							L(\vect{w}, \epsilon) &= \sum_{n=1}^{N}log(P(y^{(n)}|\vect{x^{(n)}}, \vect{w}, \epsilon)) = 
							 ...
							 \\
							\nabla_{\vect{w}}L(\vect{w}, \epsilon) &= \sum_{n=1}^{N}\frac{(1 - \epsilon)\sigma^2(v^{(n)}) exp(-v^{(n)}) y^{(n)}\vect{x}^{(n)T}} {P(y^{(n)}|\vect{x^{(n)}}, \vect{w}, \epsilon)}				
							\\
							\frac{\partial L(\vect{w}, \epsilon)}{\partial \epsilon} &= \sum_{n=1}^{N}\frac{-\sigma(v^{(n)}) + \frac{1}{2}} {P(y^{(n)}|\vect{x^{(n)}}, \vect{w}, \epsilon)}		
						\end{align*}
						...
						\begin{center}
							...
							\end{tabular}
						\end{center}
						As we can see in each case the absolute error doesn't exceed 0.01. As sanity check I use the fact that test case is symmetric in terms of $w_{1}$ and $w_{2}$, so our derivatives and finite differences must be the same for them.\\
